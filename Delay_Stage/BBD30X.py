...
def main():
    ...
    # Set constants
    serial_num = c_char_p(b"103391384")#(b"103632891")
    ...
    try:
        ...
        if lib.TLI_BuildDeviceList() == 0:
            ...
            lib.BMC_RequestPosition(serial_num, 1)
            time.sleep(0.2)
            dev_pos = c_int(lib.BMC_GetPosition(serial_num, 1))
            ...
            new_pos_real = c_double(200.0)  # in real units
            new_pos_dev = c_int()
            ...
            lib.BMC_SetMoveAbsolutePosition(serial_num, 1, new_pos_dev)
            time.sleep(10)
            lib.BMC_MoveAbsolute(serial_num, 1)

            # Close the device
            lib.BMC_Close(serial_num, 1)
